...
/* Token del almacén Blob. Vercel lo llama BLOB_READ_WRITE_TOKEN por defecto, pero al
   conectar el almacén se puede elegir otro prefijo; se acepta cualquier variable cuyo
   nombre termine en READ_WRITE_TOKEN y cuyo valor sea un token de Vercel Blob. */
function detectarToken() {
  if (process.env.BLOB_READ_WRITE_TOKEN) return { nombre: "BLOB_READ_WRITE_TOKEN", token: process.env.BLOB_READ_WRITE_TOKEN };
  const candidatos = Object.keys(process.env).filter((k) => /READ_WRITE_TOKEN$/i.test(k) && /^vercel_blob_rw_/i.test(process.env[k] || ""));
  if (candidatos.length) return { nombre: candidatos[0], token: process.env[candidatos[0]] };
  return { nombre: null, token: null };
}
const TOKEN_BLOB = detectarToken();
const USAR_BLOB = Boolean(TOKEN_BLOB.token);
...
function exigirAlmacen() {
  if (!USAR_BLOB && process.env.VERCEL) {
    throw new Error("El almacén Blob no está conectado al proyecto: no hay ninguna variable *_READ_WRITE_TOKEN con un token de Vercel Blob. En Vercel, Storage → el almacén → Connect Project, y luego Redeploy.");
  }
}
...
async function leerTexto(ruta) {
  ...
  if (USAR_BLOB) {
    ...
    const r = await conAcceso((acceso) => get(ruta, { access: acceso, useCache: false, token: TOKEN_BLOB.token }));
...
async function escribirTexto(ruta, texto) {
  ...
  if (USAR_BLOB) {
    ...
    await conAcceso((acceso) => put(ruta, texto, { access: acceso, addRandomSuffix: false, allowOverwrite: true, contentType: "text/plain", token: TOKEN_BLOB.token }));
    return;
  }
  const archivo = path.join(DIR_LOCAL, ruta);
...
async function listarRutas(prefijo) {
  ...
  if (USAR_BLOB) {
    ...
    let cursor;
    do {
      const r = await list({ prefix: prefijo, cursor, limit: 1000, token: TOKEN_BLOB.token });
...
async function verificar() {
  ...
  return { almacen: USAR_BLOB ? "Vercel Blob" : "archivos locales", variable: TOKEN_BLOB.nombre, acceso: USAR_BLOB ? ACCESO : "n/a" };
}

module.exports = { leerRegistro, guardarRegistro, listarRutas, verificar, USAR_BLOB, TOKEN_BLOB };
