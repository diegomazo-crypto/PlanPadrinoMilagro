...
function claveCifrado() {
  const valor = (process.env.PPM_CLAVE_CIFRADO || "").trim();
  if (/^[0-9a-fA-F]{64}$/.test(valor)) return Buffer.from(valor, "hex");
  // Cualquier otro secreto suficientemente largo se convierte en clave de 32 bytes.
  if (valor.length >= 16) return crypto.createHash("sha256").update(valor, "utf8").digest();
  if (EN_PRODUCCION) throw new Error("PPM_CLAVE_CIFRADO no está definida o es demasiado corta (mínimo 16 caracteres).");
...
function estadoConfiguracion() {
  const valor = (process.env.PPM_CLAVE_CIFRADO || "").trim();
  return {
    claveCifrado: /^[0-9a-fA-F]{64}$/.test(valor) ? "hexadecimal de 64 caracteres" : (valor.length >= 16 ? "texto de " + valor.length + " caracteres (derivada)" : (valor ? "demasiado corta" : "ausente")),
    secretoSesion: (process.env.PPM_SECRETO_SESION || "").length >= 16 ? "definido" : "ausente (se deriva de la clave de cifrado)",
    claveAdmin: (process.env.PPM_CLAVE_ADMIN || "").length >= 12 ? "definida" : "ausente o demasiado corta",
...
module.exports = { cifrar, descifrar, hashClave, verificarClave, normalizarCorreo, idDeCorreo, firmarToken, verificarToken, estadoConfiguracion };
